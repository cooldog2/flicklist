...
var api = {
  ...
  token: "3aea9009062fefbc3066d1f619e05f40" // TODO 0 add your api key
}
...
function searchMovies(searchTerm, callback) {
  ...
  // you can use the body of discoverMovies as a jumping off point
    ...
    $.ajax({
      ...
      data: {
        api_key: api.token,
        query: searchTerm    //the api uses query  WHERE CAN I FIND THE DOCUMENTATION SPECIFYING TO USE QUERY!!!
      },
      success: function(response) {
        model.browseItems = response.results;
        callback();
...
function render() {
  ...
  model.browseItems.forEach(function(movie) {
    ...
    // append the paragraph in between the title and the button
       ...
       //Discuss with TA why  could not read when rendered page?????????????????????
