...
function searchMovies(searchTerm, callback) {
  console.log("searching for movies with title of:'" + searchTerm + "' in their title...");
  ...
  // you can use the body of discoverMovies as a jumping off point
      $.ajax({
        ...
        data: {
          api_key: api.token,
          query: searchTerm              //query is the searchTerm user entered; the api uses 'query'
        },
        success: function(response) {
      model.browseItems = response.results;
      callback();
...
function render() {
  ...
  model.watchlistItems.forEach(function(movie) {
    ...
    var itemView = $("<li></li>")
      .append(title)
      .attr("class", "item-watchlist");            // TODO 3 give itemView a class attribute of "item-watchlist"
  ...
  model.browseItems.forEach(function(movie) {
    ...
    var button = $("<button></button>")
      ...
      .prop("disabled", model.watchlistItems.indexOf(movie) !== -1);      //indexOf returns the index of the movie in the watchlistItems array (if there is a movie); 
                                                                          ////if there isn't one, it returns -1 which means it didn't find anything in the array
                                                                          //evaluates to true if the movie is in the list; otherwise evaluates to false
      // TODO 2 (DONE - ABOVE)                                                          
    ...
    var description = $("<p></p>").text(movie.overview);        //add description 
